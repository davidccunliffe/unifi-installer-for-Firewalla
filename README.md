...
```
curl -s -L -C- "https://raw.githubusercontent.com/davidccunliffe/unifi-installer-for-Firewalla/main/unifi_docker_install.sh?$(date +%s)" | cat <(cat <(bash))
```
...
1. Run
```
curl -s -L "https://github.com/davidccunliffe/unifi-installer-for-Firewalla/blob/main/unifi-uninstall.sh?$(date +%s)" | bash
```
